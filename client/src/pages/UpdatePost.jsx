...
const UpdatePost = () => {
    ...
    if (id) {
      try {
        const fetchData = async () => {
          let res = await fetch(`/api/post/get?postId=${id}`, {
            method: "GET",
  ...
  const formSubmitHandler = async (e) => {
    e.preventDefault();
    try {
      let res = await fetch(`/api/post/update/${formData._id}/${currentUser._id}`, {
        method: "PUT",
  ...
  return (
    <div className="w-full min-h-screen">
      <h1 className="text-center my-4 text-3xl font-semibold">Update Post</h1>
      <form
